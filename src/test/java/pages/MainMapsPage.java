...
public class MainMapsPage extends BasePage {
	...
	public void businessTabIsDefaultChoose() {
		...
		Assert.assertEquals(selectedTabState, "true", "The Business tab is not selected by default");
	}

	public void testTheIconAtBusinessTab() {
		Assert.assertEquals(getIcon(businessIconTab), expectedIcon, "The icon is not the eye at the business tab");
	}
	...
	public void testTextColorAtBusinessAtTheArea() {
		...
		Assert.assertEquals(hex, whiteExpectedTextColorAtBusinessTab, "The text at the business tab is not white");
	}
	...
	public void testColorOfUnselectedTab() {
		...
		colorToCompare = we.getCssValue("color");
		hex = Color.fromString(colorToCompare).asHex();
		Assert.assertEquals(hex, whiteExpectedTextColor, "The color at the Business tab when not sselected, is not white");
	}
	...
	public void testMapsHeaderTextTopRight() {
		...
		Assert.assertEquals(actualMapsHeaderText, expectedMapsHeaderText, "the text at the maps Header is not right one");
	}
	...
	public void testColorOfTicketNumber() {
		...
		colorToCompare = we.getCssValue("color");
		hex = Color.fromString(colorToCompare).asHex();
		Assert.assertEquals(hex, whiteExpectedTextColorAtBusinessTab, "The number at the card at business tab is not white");
	}
	...
	public void testColorOfNumberAtTicketThatNotTheSameAsBackground() {
		...
		Assert.assertNotEquals(hex, expectedCardNumberBackgroundColor, "The background color of the number at the card is the same as the rest of the background");
	}
	...
	public void testClickOnNumberLeadToBusinessPage() {
		...
		Assert.assertEquals(actualTextOfpage, expectedTextOfCard, "The text of the first card is not the same as the text of the page");
	}

	public void testThatEachCardHasTitle() {
		driver.navigate().back();
		Assert.assertFalse (getTextFromElements(cardTitleElement).isEmpty(),"There is cards without title on it");
	}
